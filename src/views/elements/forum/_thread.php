...
<td class="podium-thread-line">
    <?php if($firstToSee):?>
        <a href="<?= Url::to(['forum/show', 'id' => $firstToSee->id]) ?>" class="podium-go-to-new pull-right" style="margin-right:10px" data-pjax="0" data-toggle="popover" data-container="body" data-placement="left"
           data-trigger="hover focus" data-html="true"
           data-content="<small><?= str_replace('"', '&quote;', StringHelper::truncateWords($firstToSee->parsedContent, 20, '...', true)) ?><br><strong><?= $firstToSee->author->podiumName ?></strong> <?= Podium::getInstance()->formatter->asRelativeTime($firstToSee->updated_at) ?></small>"
           title="<?= Yii::t('podium/view', 'First New Post') ?>">
